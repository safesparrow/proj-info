...
let findByPath path parsed =
    ...
    |> function
        | Some x -> x
        | None -> failwithf $"key '%s{path}' not found in %A{parsed |> Array.map (fun kv -> kv.Key)}"
...
let withLog name f test =
    test name (fun () ->
        ...
        let logger = Log.create $"Test '%s{name}'"
...
module ExpectNotification =
    ...
    let failed (name: string) =
        ...
        $"failed %s{name}", isFailed

    let expectNotifications actual expected =

        let getMessages =
            function
            | WorkspaceProjectState.Loading path -> $"loading %s{path} "
            | WorkspaceProjectState.Loaded (po, _, _) -> $"loaded %s{po.ProjectFileName}"
            | WorkspaceProjectState.Failed (path, _) -> $"failed %s{path}"

        Expect.equal (List.length actual) (List.length expected) $"expected notifications: %A{expected |> List.map fst} \n actual notifications: - %A{actual |> List.map getMessages}"

        expected
        |> List.iter (fun (name, f) ->
            let item = actual |> List.tryFind (fun a -> f a)
            let minimal_info = item |> Option.map getMessages |> Option.defaultValue ""
            Expect.isSome item $"expected %s{name} but was %s{minimal_info}")


    type NotificationWatcher(loader: IWorkspaceLoader, log) =
...
let testLegacyFrameworkProject toolsPath workspaceLoader isRelease (workspaceFactory: ToolsPath * (string * string) list -> IWorkspaceLoader) =
    ptestCase
    |> withLog $"can load legacy project - %s{workspaceLoader} - isRelease is %b{isRelease}" (fun logger fs ->
        ...
        [ loading projPath; loaded projPath ] |> expectNotifications watcher.Notifications
        ...
        let expectedSources = [ projDir / "Project1A.fs" ] |> List.map Path.GetFullPath
...
let testLegacyFrameworkMultiProject toolsPath workspaceLoader isRelease (workspaceFactory: ToolsPath * (string * string) list -> IWorkspaceLoader) =
    ...
    |> withLog $"can load legacy project - %s{workspaceLoader} - isRelease is %b{isRelease}" (fun logger fs ->
        ...
        let n1ExpectedSources = [ projDir / "MultiProject1.fs" ] |> List.map Path.GetFullPath

        let l1Parsed = parsed |> expectFind l1 "the F# lib"
        let l1ExpectedSources = [ l1Dir / "Project1A.fs" ] |> List.map Path.GetFullPath

        let l2Parsed = parsed |> expectFind l2 "the F# exe"
        let l2ExpectedSources = [ l2Dir / "Project1B.fs" ] |> List.map Path.GetFullPath
...
let testSample2 toolsPath workspaceLoader isRelease (workspaceFactory: ToolsPath * (string * string) list -> IWorkspaceLoader) =
    testCase
    |> withLog $"can load sample2 - %s{workspaceLoader} - isRelease is %b{isRelease}" (fun logger fs ->
...
let testSample3 toolsPath workspaceLoader (workspaceFactory: ToolsPath -> IWorkspaceLoader) expected =
    testCase
    |> withLog $"can load sample3 - %s{workspaceLoader}" (fun logger fs ->
        ...
        Expect.equal parsed.Length 3 $"console (F#) and lib (F#) and lib (C#), but was %A{parsed |> List.map (fun x -> x.ProjectFileName)}"
...
let testSample4 toolsPath workspaceLoader (workspaceFactory: ToolsPath -> IWorkspaceLoader) =
    testCase
    |> withLog $"can load sample4 - %s{workspaceLoader}" (fun logger fs ->
        ...
        Expect.equal parsed.Length 1 $"multi-tfm lib (F#), but was %A{parsed |> List.map (fun x -> x.ProjectFileName)}"
...
let testSample5 toolsPath workspaceLoader (workspaceFactory: ToolsPath -> IWorkspaceLoader) =
    testCase
    |> withLog $"can load sample5 - %s{workspaceLoader}" (fun logger fs ->
...
let testLoadSln toolsPath workspaceLoader (workspaceFactory: ToolsPath -> IWorkspaceLoader) expected =
    testCase
    |> withLog $"can load sln - %s{workspaceLoader}" (fun logger fs ->
...
let testSample9 toolsPath workspaceLoader (workspaceFactory: ToolsPath -> IWorkspaceLoader) =
    testCase
    |> withLog $"can load sample9 - %s{workspaceLoader}" (fun logger fs ->
...
let testRender2 toolsPath workspaceLoader (workspaceFactory: ToolsPath -> IWorkspaceLoader) =
    testCase
    |> withLog $"can render sample2 - %s{workspaceLoader}" (fun logger fs ->
...
let testRender3 toolsPath workspaceLoader (workspaceFactory: ToolsPath -> IWorkspaceLoader) =
    testCase
    |> withLog $"can render sample3 - %s{workspaceLoader}" (fun logger fs ->
...
let testRender4 toolsPath workspaceLoader (workspaceFactory: ToolsPath -> IWorkspaceLoader) =
    testCase
    |> withLog $"can render sample4 - %s{workspaceLoader}" (fun logger fs ->
...
let testRender5 toolsPath workspaceLoader (workspaceFactory: ToolsPath -> IWorkspaceLoader) =
    testCase
    |> withLog $"can render sample5 - %s{workspaceLoader}" (fun logger fs ->
...
let testRender8 toolsPath workspaceLoader (workspaceFactory: ToolsPath -> IWorkspaceLoader) =
    testCase
    |> withLog $"can render sample8 - %s{workspaceLoader}" (fun logger fs ->
...
let testProjectNotFound toolsPath workspaceLoader (workspaceFactory: ToolsPath -> IWorkspaceLoader) =
    testCase
    |> withLog $"project not found - %s{workspaceLoader}" (fun logger fs ->
...
let testFCSmap toolsPath workspaceLoader (workspaceFactory: ToolsPath -> IWorkspaceLoader) =
    testCase
    |> withLog $"can load sample2 with FCS - %s{workspaceLoader}" (fun logger fs ->
        ...
        let expectP2PKeyIsTargetPath (pos: Map<string, ProjectOptions>) fcsPo =
            for tar, fcsPO in allP2P fcsPo do
                ...
                Expect.equal tar dpoPo.TargetPath $"p2p key is TargetPath, fsc projet options was '%A{fcsPO}'"
        ...
        Expect.isEmpty result.Diagnostics $"no errors but was: %A{result.Diagnostics}"
...
let testFCSmapManyProj toolsPath workspaceLoader (workspaceFactory: ToolsPath -> IWorkspaceLoader) =
    testCase |> withLog $"can load sample3 with FCS - %s{workspaceLoader}" (fun logger fs ->
...
let testSample2WithBinLog toolsPath workspaceLoader (workspaceFactory: ToolsPath -> IWorkspaceLoader) =
    testCase |> withLog $"can load sample2 with bin log - %s{workspaceLoader}" (fun logger fs ->
...
module ExpectProjectSystemNotification =
    ...
    let loadedFrom (name: string) isCached =
        let isLoaded n =
            match n with
            | ProjectResponse.Project (po, fromCache) when po.ProjectFileName.EndsWith(name) && fromCache = isCached -> true
            | _ -> false

        $"loaded %s{name} %b{isCached}", isLoaded
    ...
    let changed (name: string) =
        ...
        $"changed %s{name}", isFailed

    let expectNotifications actual expected =
        let getMessage =
            function
            | ProjectResponse.ProjectLoading path -> $"loading %s{Path.GetFileName path}"
            | ProjectResponse.Project (po, fromCache) -> $"loaded %s{Path.GetFileName po.ProjectFileName} %b{fromCache}"
            | ProjectResponse.ProjectError (path, _) -> $"failed %s{Path.GetFileName path}"
            | ProjectResponse.WorkspaceLoad finished -> $"workspace %b{finished}"
            | ProjectResponse.ProjectChanged projectFileName -> $"changed %s{Path.GetFileName projectFileName}"

        Expect.equal (List.length actual) (List.length expected) $"expected notifications: %A{expected |> List.map fst}\n actual notifications %A{actual |> List.map getMessage}"
        ...
        |> List.iter (fun (n, check) ->
            ...
            Expect.isTrue (f n) $"expected %s{name} but was %s{minimal_info}")
...
let testLoadProject toolsPath =
    testCase
    |> withLog "can use getProjectInfo" (fun logger fs ->
...
let testProjectSystem toolsPath workspaceLoader workspaceFactory =
    testCase
    |> withLog $"can load sample2 with Project System - %s{workspaceLoader}" (fun logger fs ->
...
let testProjectSystemCacheLoad toolsPath workspaceLoader workspaceFactory =
    testCase
    |> withLog $"project system can reload from cache - %s{workspaceLoader}" (fun logger fs ->
        ...
        let result2 = controller.LoadProject(projPath) |> Async.RunSynchronously

        Expect.isTrue result2 "load succeeds"
        let parsed2 = controller.ProjectOptions |> Seq.toList |> List.map snd
        ...
        Expect.isEmpty result2.Diagnostics $"no errors but was: %A{result2.Diagnostics}"
...
let testProjectSystemOnChange toolsPath workspaceLoader workspaceFactory =
    testCase
    |> withLog $"can load sample2 with Project System, detect change on fsproj - %s{workspaceLoader}" (fun logger fs ->
...
let debugTests toolsPath workspaceLoader (workspaceFactory: ToolsPath -> IWorkspaceLoader) =
    ptestCase
    |> withLog $"debug - %s{workspaceLoader}" (fun logger fs ->
